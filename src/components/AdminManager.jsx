...
import { Plus, Trash2, Edit2, Upload, Check, X } from 'lucide-react';

export default function AdminManager() {
  const store = useStore();
  const defaultRappers = store.defaultRappers || [];
  ...
  const handleAddSubmit = (e) => {
    e.preventDefault();
    if (!addName) return;
    store.addDefaultRapper({ name: addName, image: addImage });
    setAddName('');
    setAddImage('');
  };
  ...
  const handleUpdate = () => {
    if (!editName) return;
    store.updateDefaultRapper(editingId, { name: editName, image: editImage });
    setEditingId(null);
  };

  const handleDelete = (id) => {
    store.deleteDefaultRapper(id);
  };

  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
        <h2 style={{ margin: 0, color: 'var(--danger)' }}>Quản trị Data Gốc (Admin)</h2>
      </div>
      ...
      <form onSubmit={handleAddSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '25px', paddingBottom: '20px', borderBottom: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        <button type="submit" className="btn btn-primary" disabled={!addName}>
          <Plus size={16} /> Thêm Rapper Gốc
        </button>
      </form>

      <div className="list">
        {defaultRappers.map(r => (
          editingId === r.id ? (
            ...
            <div key={r.id} className="list-item">
              <div className="list-item-content">
                {r.image ? <img src={r.image} alt={r.name} className="avatar" /> : <div className="avatar" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--danger)' }}>{r.name?.[0] || '?'}</div>}
